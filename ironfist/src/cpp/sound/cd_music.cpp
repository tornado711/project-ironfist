...
//maybe put this in soundManager ctor? //soundManager::CDStartup(soundManager *this)
bool ensure_bass_init() {
...
int __fastcall SystemOptionsHandler(struct tag_message &msg) {
  gpSoundManager->field_6A6 = 1;
  gpSoundManager->field_69E = 1;
  ensure_bass_init();
...
//put in soundManager dtor? //soundManager::CDShutdown(soundManager *this)
void cleanup_bass()  {
  BASS_Free();
}
...
void soundManager::CDSetVolume(int unknown_control_code, int unknown) {
  float volume = ((11 - giMusicVolume) / 10.0f);
  if (giMusicVolume == 0)
  {
    global_volume = 0;
    CDStop();
  ...
  if (global_volume == 0 && giMusicVolume == 1) //going from 'Off' to 'On'
  {
